...
| `/riprap:review-loop` | Driving `/riprap:review` and its fixes through bounded cycles, then stopping at the merge gate. Its Claude Code half is a payload workflow, not a plugin file — `plugin/payload/.claude/workflows/riprap-review-loop.js` — so a change to the procedure moves two files, and the skill is the definition. |
