...
| `/riprap:vet` | Driving `/riprap:review` and its fixes through bounded cycles, then stopping at the merge gate. Its Claude Code half is a **payload workflow**, not a plugin file — `plugin/payload/.claude/workflows/riprap-vet.js` — so a change to the procedure moves two files, and the skill is the definition. The workflow names its angles but never restates their questions; each reviewer reads its row from `/riprap:review`'s own table, because a copy here would drift with nothing able to notice. |
